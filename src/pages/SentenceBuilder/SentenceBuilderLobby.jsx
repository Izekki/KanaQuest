...
function getDifficultyConfig(level) {
  switch (level) {
    case 1:
      return { label: 'Básico (N5)', color: 'bg-emerald-50 text-emerald-700 border-emerald-200', icon: 'sprout' };
    case 2:
      return { label: 'Elemental (N5+)', color: 'bg-blue-50 text-blue-700 border-blue-200', icon: 'sprout' };
    case 3:
      return { label: 'Intermedio (N4)', color: 'bg-amber-50 text-amber-700 border-amber-200', icon: 'sparkles' };
    default:
      return { label: `Nivel ${level}`, color: 'bg-purple-50 text-purple-700 border-purple-200', icon: 'star-mastery' };
  }
}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <div className="flex items-center gap-2.5 mt-1 sm:mt-2">
            <Icon name="puzzle-blocks" className="w-7 h-7 text-[#6b2832]" />
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-[rgb(var(--color-neutral))]">
        ...
        {!loading && topics.length > 0 && (
          <div className="flex items-center gap-3 self-start rounded-2xl border border-[#eaded6] bg-white/80 px-4 py-2.5 shadow-sm backdrop-blur-sm sm:self-auto">
            <Icon name="book-open" className="w-5 h-5 text-accent" />
              ...
              >
                <div>
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    >
                      <Icon name={diffConfig.icon} className="w-3.5 h-3.5" />
